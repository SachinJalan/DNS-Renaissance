...
def get_resolution_times(domain, qclass, qtype):
    ...
    dig_trace_command = ['dig', '+trace', '-c', qclass, '-t', qtype, domain]
    dig_command_iterative = ['dig', '@8.8.8.8', '+norecurse', '-c', qclass, '-t', qtype, domain]
    dig_command_recursive = ['dig', '@8.8.8.8', '+recurse', '-c', qclass, '-t', qtype, domain]
    ...
    # DNS Trace to find the authoritative NS server's IP address
    result_trace = subprocess.run(dig_trace_command, stderr=subprocess.PIPE, stdout=subprocess.PIPE, text=True)
    lines = result_trace.stdout.split("\n")

    # Find the last authoritative NS server IP (improved method with regex)
    ns_server_ip = None
    for line in lines[-1::-1]:
        if 'Received' in line and '#' in line:
            # Extract the IP address from the line
            match = re.search(r'(\d+\.\d+\.\d+\.\d+)#53', line)
            if match:
                ns_server_ip = match.group(1)
    ...
    if ns_server_ip:
        ...
        if time_match:
            ...
            avg_ping_time = sum(float(t) for t in time_match) / len(time_match)
            print(f"PING TIME (avg): {avg_ping_time} ms, IP: {ns_server_ip}")
            time_delegated = 0.5 * avg_ping_time + 0.5 * time_recursive if time_recursive else None
        else:
            print("No ping time found.")
    else:
...
def save_results_to_csv(results, output_file):
    ...
    headers = ["master", "recursive", "iterative", "delegated","domains", "count"]
    print(results)
    # Write the CSV file
...
def main():
    ...
    output_file = "results-time-test.csv"
    # analyzed_domains = []
